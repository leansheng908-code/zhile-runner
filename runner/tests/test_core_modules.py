...
def test_motion_tag_extraction():
    """测试P0.58 Live2D动作标签提取"""
    import re
    _RE = re.compile(r'\{([me]):([^}]+)\}')

    # 测试1：混合动作+表情标签
    text = "{m:wave}你好呀～{e:happy}今天真好呢"
    motions = []
    clean_parts = []
    pos = 0
    last_end = 0
    for match in _RE.finditer(text):
        ...
        motions.append({
            "type": "motion" if match.group(1) == "m" else "expression",
    ...
    assert clean_text == "你好呀～今天真好呢"
    assert len(motions) == 2
    assert motions[0] == {"type": "motion", "name": "wave", "pos": 0}
    assert motions[1] == {"type": "expression", "name": "happy", "pos": 4}

    # 测试2：无标签的文本不受影响
    plain = "普通的文字而已"
    assert _RE.sub('', plain) == plain

    # 测试3：strip_motion_tags静态方法
    tagged = "{m:nod}嗯嗯{m:tilt_head}什么呢"
    stripped = _RE.sub('', tagged)
    assert stripped == "嗯嗯什么呢"

    # 测试4：标签不被TTS读取（模拟speak清洗流程）
    tts_input = "{m:shy}才、才没有想你呢"
    tts_clean = _RE.sub('', tts_input)
    assert "{" not in tts_clean  # 标签已完全剥离
    assert "才、才没有想你呢" in tts_clean
